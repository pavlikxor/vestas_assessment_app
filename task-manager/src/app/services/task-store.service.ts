  ...
  loadTasks = rxMethod<string | void>(
    pipe(
      ...
      exhaustMap(filterArg => {
        const filter =
          (typeof filterArg === 'string' ? filterArg : '').toLowerCase();
        ...
        this.taskApiService
          ...
          .pipe(
            tapResponse({
              ...
              error: (err: HttpErrorResponse) =>
                this.notificationService.error(
                  `Failed to update task "${updatedTask.name}": 
          ...
          .pipe(
            tapResponse({
              ...
              finalize: () => {
                patchState(this.state, { isLoading: false });
                this.notificationService.success(
                  `Task "${task?.name ?? id}" status updated to "${TASK_STATUS_LABELS[newStatus] || newStatus}" successfully`
                );
              },
            })
          );
  ...
  addTask = rxMethod<CreateTask>(
    pipe(
      ...
      exhaustMap(newTask => {
        ...
        return this.taskApiService
          .saveTasks([...this.state.tasks(), task])
          .pipe(
            tapResponse({
              next: () => {
              ...
              error: (err: HttpErrorResponse) =>
                this.notificationService.error(
                  `Failed to add task "${newTask.name}": ` +
                    (err?.message || err?.error || err)
                ),
              finalize: () => {
                patchState(this.state, { isLoading: false });
  ...
  deleteTask = rxMethod<string>(
    pipe(
      ...
      exhaustMap(id => {
        const task = this.state.tasks().find(t => t.id === id);
        return this.taskApiService
          .saveTasks(this.state.tasks().filter(t => t.id !== id))
          .pipe(
            tapResponse({
              next: () => {
                const updatedTasks = this.state
              ...
              error: (err: HttpErrorResponse) =>
                this.notificationService.error(
                  `Failed to delete task "${task?.name ?? id}": ` +
                    (err?.message || err?.error || err)
                ),
              finalize: () => {
                patchState(this.state, { isLoading: false });
                this.notificationService.success(
                  `Task "${task?.name ?? id}" deleted successfully`
                );
              },
            })
          );
